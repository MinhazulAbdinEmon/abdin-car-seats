 ...
 */
export default function WhatsAppGlowButton({ label = "WhatsApp", className = "", compact = false }) {
  return (
    <a
      href={whatsappLink()}
      target="_blank"
    ...
    >
      ...
      {/* inner button face */}
      <span
        className={`relative z-10 inline-flex items-center gap-2 rounded-full bg-ink font-semibold text-cream transition-colors duration-300 group-hover:bg-[#0c1410] ${
          compact ? "h-12 w-12 justify-center" : "px-5 py-2.5 text-[0.85rem]"
        }`}
      >
        <svg viewBox="0 0 24 24" className={`fill-[#25D366] ${compact ? "h-6 w-6" : "h-4 w-4"}`} aria-hidden="true">
