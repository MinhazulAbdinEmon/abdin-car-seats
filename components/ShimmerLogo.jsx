...
export default function ShimmerLogo({
  ...
  return (
    <span className={`relative isolate inline-block ${className}`}>
      ...
      {/* shimmer sheen, clipped to the logo silhouette (desktop only — mask
          overlays can obscure the logo on some mobile browsers) */}
      <span
        aria-hidden
        className="pointer-events-none absolute inset-0 z-20 hidden animate-[shimmer2_3.5s_infinite_linear] bg-[linear-gradient(110deg,transparent_30%,rgba(255,255,255,0.65)_47%,rgba(170,215,255,0.55)_53%,transparent_70%)] [background-size:200%_100%] md:block"
        style={mask}
      />
    </span>
  );
